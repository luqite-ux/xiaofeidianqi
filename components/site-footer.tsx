...
import { inquiryPath, productPath } from '@/lib/routes'
  ...
  return (
    <footer className="border-t border-line bg-white px-5 py-12 sm:px-8">
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.2fr_1fr_1fr]">
        ...
        <div>
          <p className="font-bold text-ink">Product Systems</p>
          <div className="mt-4 grid gap-3 text-sm text-muted">
            {productCategories.slice(0, 5).map((item) => (
              <Link key={item.slug} href={productPath(item.slug)} className="hover:text-brand-blue">
                {item.title.en}
              </Link>
            ))}
          </div>
      ...
      <div className="mx-auto mt-10 max-w-7xl border-t border-line pt-6 text-sm text-muted">
        © 2026 {siteInfo.company}. All rights reserved.
      </div>
    </footer>
  )
}
